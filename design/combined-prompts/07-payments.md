# Payments Combined Execution Prompt

<!-- markdownlint-disable MD013 MD012 MD024 MD025 -->

Use this file as a standalone prompt for generating the Payments page. It already includes the shared product context, reviewed consumer UI direction, page requirements, and page-specific generation instructions.
...
Purpose: manage Filecoin Pay account state needed for storage.
...
- approvals
- payment rails
- debt or required top-up
- Mainnet/Calibration-specific state

This page should read as storage funding and account readiness for users. Payment rails and validator details can appear in advanced sections.
...
```text
Design a Payments page for Synapse.

Use `images/payments-v1.png` as the visual reference when available.

Show Filecoin Pay account state for the selected network:
- available balance
- funds
- lockup
- lockup rate
...
Make Mainnet/Calibration context visible. The page should clearly show when the user is on Calibration versus Mainnet.

Lead with user-facing storage balance, available funds, required top-up, and readiness. Keep payment rail internals in a details or advanced section.

The page should help the user understand whether uploads can proceed now. Payment rails, lockup, and approvals should support that readiness story, not dominate it.
```
